...
import { Addproduct, card, filter, productpage, smallCard } from "../../Consts/styles";
const { width } = Dimensions.get("screen");
const { height } = Dimensions.get("screen");

const cardwidth = width / 2;
let iconcolor;
const Recycle = ({ navigation, route }) => {
  ...
  const styles = Addproduct(COLORS)
  ...
  const handleProductPress = (product) => {
    navigation.navigate("RecycleDetails", { product, COLORS: COLORS });
  };
    ...
    <View style={productpages.container}>
                ...
                renderButton={(selectedItem, isOpened) => {
                  return (
                    <View style={filterr.dropdownButtonStyle}>
                      ...
                      <Icon name={isOpened ? 'chevron-up' : 'chevron-down'} style={filterr.dropdownButtonArrowStyle} />
                    </View>
                  );
                }}
      ...
      <View style={styles.Addproduct}>
        <TouchableOpacity onPress={() => navigation.navigate('AddUserProduct')}
          ...
          <Text style={{ color: COLORS.white, fontWeight: 'bold' }}>Add product</Text>
        </TouchableOpacity>
      </View>
